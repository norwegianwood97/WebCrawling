...
const warnFailures = (stepName: string, failures: string[]): void => {
...
export const clickByTextCandidates = async (
  ...
  for (const text of texts) {
    try {
      const target = await visibleFirst(page.getByText(text, { exact: false }));
      if (!target) {
...
export const clickBySelectorCandidates = async (
  ...
  for (const selector of selectorCandidates) {
    try {
      const target = await visibleFirst(page.locator(selector));
      if (!target) {
...
export const clickByCandidates = async (
  ...
  warnFailures(stepName, failures);
  return { success: false, failures };
};
