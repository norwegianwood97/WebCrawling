...
const gotoWithSafety = async (
  ...
  if (status === 403 || status === 429) {
    ...
    throw new Error(
      `${stepName}: HTTP ${status} 응답을 받아 크롤링을 중단합니다.`,
    );
  }
...
const openRecruitPage = async (
  ...
  // 중요:
  // getByText("채용정보")는 배너/추천 링크/다른 영역의 "채용정보"를 잘못 클릭할 수 있다.
  // 그래서 실제 수집 대상인 IT개발·데이터 직업별 페이지로 직접 이동한다.
...
const isJobCategoryAlreadyApplied = async (page: Page): Promise<boolean> => {
  ...
  return (
    /IT개발·데이터/.test(bodyText) && /(전체|선택|직업별|개발)/.test(bodyText)
  );
};
...
const selectJobCategory = async (
  ...
  if (await isJobCategoryAlreadyApplied(page)) {
    console.log(
      "IT개발·데이터 조건이 이미 적용된 것으로 보여 직업 선택 UI 조작을 생략합니다.",
    );
    return;
  }
...
const selectCareer = async (
  ...
  console.log(
    `#btn_check_career_over0 연결 label 텍스트: ${labelText || "(확인 실패)"}`,
  );
