...
import { JobPosting } from "../crawler/types.js";
...
const xlsxColumns: XlsxColumn[] = [
  ...
  { header: "상세 전체본문", key: "detail_text", width: 80, wrapText: true },
...
export const writeJson = async (jobs: JobPosting[], outputDir: string): Promise<string> => {
  await mkdir(outputDir, { recursive: true });
  const filePath = path.join(outputDir, "saramin_agent_jobs.json");
  await writeFile(filePath, `${JSON.stringify(serializeJobs(jobs), null, 2)}\n`, "utf8");
  return filePath;
};

export const writeCsv = async (jobs: JobPosting[], outputDir: string): Promise<string> => {
  await mkdir(outputDir, { recursive: true });
  const filePath = path.join(outputDir, "saramin_agent_jobs.csv");
...
export const writeXlsx = async (
  ...
  const fileName = timestampFileName
    ? `saramin_agent_jobs_${timestampForFileName()}.xlsx`
    : "saramin_agent_jobs.xlsx";
  const filePath = path.join(outputDir, fileName);

  const workbook = new ExcelJS.Workbook();
  workbook.creator = "saramin-agent-crawler";
  ...
  try {
    ...
    if (isBusyError(error)) {
      throw new Error(
        "XLSX 파일이 열려 있어서 저장할 수 없습니다. 엑셀에서 파일을 닫고 다시 실행해주세요.",
      );
    }
